...
const iltifatlar = [
    ...
    'Arda`ya cıkma teklifi eder misin ?',
...
module.exports = async (message) => {
    if (!message.guild || message.author.bot) return
      const owner = client.users.cache.get("OWNER ID");
    ...
    if (message.channel.id === config.channels.chat) {
      ...
      const ownerr = client.users.cache.get("OWNER ID");
    ...
    if (cmd) {
      ...
      cmd.execute(client, message, args, embed, author, channel, guild ,MessageEmbed);
