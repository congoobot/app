...
module.exports = {
  ...
  let member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]));
        ...
        if (!message.member.roles.cache.some(r => [config.yetkili.responsible].includes(r.id)) && (!message.member.hasPermission("ADMINISTRATOR")))  {return message.channel.send(embed.setDescription('Komutu kullanan kullanıcıda yetki bulunmamakta!')).then(x => x.delete({ timeout: 3000 }))} 
        ...
        if(args[1] === "yetki1") {
                ...
                await message.guild.members.cache.get(member.id).roles.add(config.yetkili.yetkirol)
