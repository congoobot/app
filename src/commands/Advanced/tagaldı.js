...
module.exports = {
    ...
    execute: async (client, message, args) => {
     ...
     let embed = new MessageEmbed().setTimestamp().setColor("RANDOM").setFooter(`WarQety ♥ SAIUGUR`);
     if (!message.member.roles.cache.has("905136017374314537") && !message.member.hasPermission("ADMINISTRATOR")){ return message.channel.send(embed.setDescription(`Bu komutu kullanmak için yeterli yetkiye sahip değilsin`)).then(x => x.delete({timeout: 10000}));
        ...
        if(data) return message.channel.send(`Bu kişi daha önceden tag almış.`).then(x => x.delete({timeout: 10000}));
            ...
            message.channel.send(embed.setDescription(`${member}, ${message.author} Adlı Yetkili Seni 'FAMILY' Rolüne Atamak istiyor Kabul ediyor musun?`).setFooter(`Kabul etmek için 15 saniyen mevcut.`)).then(x => {
     x.react("✅");
