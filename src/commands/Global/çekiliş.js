...
module.exports = {
  ...
  let prefix = await prefixDB.get(`prefix`) || config.bot.prefix
  let tag = await db.get(`config.registration.GuilDTag`)
  let booster = await db.get(`config.rolse.boosterrole`)
...
**Kazanan Kullanıcı:** <@${random}> (\`${random}\`)`))
    ...
    let Embed = new Discord.MessageEmbed()
      ...
      .setDescription(`
**Çekilişe katılmak için** :tada: **tepkisine tıklayınız** (İsminde ✭ tagını bulunduran üyelerin isimleri 2 defa yazılmaktadır!)
...
**Verilecek Ödül:** \`${prize.replace(message.mentions.roles.first(), "")}\`
      `)
      .setFooter(client.config.ayarlar.footer)
      .setTimestamp()
      .setColor(`BLUE`);
